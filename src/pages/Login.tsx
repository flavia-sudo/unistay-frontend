...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      dispatch(login({ ...user, token })); // authSlice handles all localStorage
      ...
      if (axios.isAxiosError(error)) {
        setErrorMessage(error.response?.data?.message || error.response?.data?.error || "Login failed");
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 pt-20">
      <div className="max-w-md w-full p-8 bg-white rounded-lg shadow-md space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" className="w-full py-2 text-white bg-purple-800 rounded-md hover:bg-purple-700 font-semibold shadow-lg transition-colors">
            Sign In
          </button>
        </form>
        ...
        <p className="text-sm text-center text-gray-600">
          ...
          <button onClick={() => navigate("/register")} className="text-purple-600 underline">Sign Up</button>
        </p>
      </div>
    </div>
  );
