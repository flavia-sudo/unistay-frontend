...
const Users = () => {
  ...
  const filteredUsers = useMemo(() => {
    return users.filter((user) => {
      ...
      const matchesRoles =
        roleFilter === "all" ||
        (roleFilter === "Student" && roleLower === "student") ||
        (roleFilter === "Landlord" && roleLower === "landlord") ||
        (roleFilter === "Admin" && roleLower === "admin");
  ...
  const landlords = users.filter((user) => user.role.toLowerCase() === "landlord");
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white/80 backdrop-blur border border-slate-200 rounded-2xl shadow-sm p-4 mb-8 flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <select
            value={roleFilter}
            onChange={(e) =>
              setRoleFilter(e.target.value as "all" | "Student" | "Landlord" | "Admin")
            }
              ...
              <tbody>
                {filteredUsers.map((user) => (
                  <tr key={user.userId} className="border-b last:border-none">
                    <td className="py-4 font-medium text-slate-900">{user.firstName}</td>
                    <td className="py-4 font-medium text-slate-900">{user.lastName}</td>
                    ...
                    <td className="py-4 text-slate-600">{user.verified ? "Yes" : "No"}</td>
