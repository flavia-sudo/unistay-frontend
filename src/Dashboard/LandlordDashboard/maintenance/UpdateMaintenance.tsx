...
const UpdateMaintenanceLandlord = ({ maintenance }: UpdateMaintenanceProps) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  useEffect(() => {
    if (maintenance) {
      reset({ status: maintenance.status ?? "pending" });
    } else {
        ...
        {maintenance && (
          <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 mb-6 text-sm space-y-1">
            <p>
              ...
              <span className="text-slate-600">{maintenance.firstName} {maintenance.lastName}</span>
            </p>
            <p>
            ...
            <p>
              ...
              <span className="text-slate-600">{maintenance.issueTitle}</span>
            </p>
            <p>
              <span className="font-medium text-slate-700">Description:</span>{" "}
              <span className="text-slate-600">{maintenance.description}</span>
            </p>
            <p>
...
export default UpdateMaintenanceLandlord;