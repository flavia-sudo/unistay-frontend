...
const UpdateMaintenanceLandlord = ({ maintenance }: UpdateMaintenanceProps) => {
  ...
  useEffect(() => {
    if (maintenance) {
      ...
      setValue("status", maintenance.status as UpdateMaintenanceInputs["status"]);
    } else {
      reset();
    }
      ...
      <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-xl shadow-2xl p-6 w-full max-w-md z-50">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            />
            {errors.hostelId && (
              <p className="text-red-500 text-xs">{errors.hostelId.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            />
            {errors.issueTitle && (
              <p className="text-red-500 text-xs">{errors.issueTitle.message}</p>
            )}
          </div>
