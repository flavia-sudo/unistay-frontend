...
const ViewReview = () => {
  ...
  const {
    ...
    isLoading,
    error,
  } = reviewsAPI.useGetReviewsQuery(undefined, {
  ...
  const landlordReviews = landlordId
    ? reviews.filter((r) => r.landlordId === landlordId)
    : [];

  const [search, setSearch] = useState("");

  const filteredReviews = useMemo(() => {
    return landlordReviews.filter((review) =>
      `${review.hostelName} ${review.comment} ${review.firstName} ${review.lastName}`
        .toLowerCase()
        .includes(search.toLowerCase())
    );
  }, [landlordReviews, search]);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 bg-linear-to-r from-amber-500 to-orange-600 rounded-2xl shadow-sm px-8 py-10">
          ...
          <p className="text-orange-100 mt-3">
            View all reviews made on your hostels
          </p>
        </div>
        ...
        <div className="mb-6 bg-orange-50 border border-orange-100 rounded-2xl shadow-sm px-8 py-6">
          ...
          <h2 className="text-4xl font-bold text-orange-700 mt-3">
