...
const ViewHostel = () => {
  ...
  useEffect(() => {
    ...
    if (storedLandlord) {
      try {
        ...
        if (parsedLandlord.landlordId) {
          setLandlordId(Number(parsedLandlord.landlordId));
        }
      } catch (error) {
  ...
  const { data: hostelData, isLoading } =
    hostelsAPI.useGetHostelsQuery(undefined, {
      refetchOnMountOrArgChange: true,
      pollingInterval: 50000,
    });

  const hostels: THostel[] = hostelData?.data || [];
  ...
  const filteredHostels = useMemo(() => {
    return landlordHostels.filter((hostel) =>
    ...
    <div className="w-full min-h-full bg-slate-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 bg-linear-to-r from-green-600 to-teal-600 text-white px-8 py-12 rounded-3xl flex items-center">
            ...
            <p className="text-green-100 mt-5 mb-4">
                ...
                {filteredHostels.map((hostel) => (
                  ...
                  >
                    ...
                    <td className="py-4 font-semibold">
                      Ksh {Number(hostel.price).toLocaleString()}
                    </td>
