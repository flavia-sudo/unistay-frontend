...
const UpdateReview = ({ review }: UpdateReviewProps) => {
    ...
    const onSubmit: SubmitHandler<UpdateReviewInputs> = async (data) => {
        ...
        try {
            // ✅ .unwrap() so the catch block actually fires on API errors
            await updateReview({ reviewId: review.reviewId, ...data }).unwrap();
    ...
    const handleClose = () => {
        reset(); // ✅ Reset form on cancel so stale values don't persist
        (document.getElementById("update_modal") as HTMLDialogElement)?.close();
    };
            ...
            <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-xl shadow-2xl p-6 w-full max-w-md z-50">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div className="form-control w-full">
                        ...
                        {/* ✅ min/max enforced at both HTML and schema level */}
                        <input
                            type="number"
                            min={1}
                            max={5}
