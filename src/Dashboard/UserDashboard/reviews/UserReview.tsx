...
type TReviewWithRelations = TReview & { hostelName?: string };

const UserReview = () => {
  const userId = useSelector((state: RootState) => state.auth.user?.userId);

  const { data: response, isLoading: reviewsLoading, error: reviewsError } =
    ...
    <div className="p-6 min-h-screen bg-slate-50">
      <div className="max-w-7xl mx-auto">
        ...
        {reviews.length > 0 ? (
                ...
                {reviews.map((review) => (
                  <tr key={review.reviewId} className="border-b last:border-none">
                    ...
                    <td className="px-4 py-3 flex gap-2">
                      <button onClick={() => handleEdit(review)} className="bg-blue-600 hover:bg-blue-700 text-white p-2 rounded-md shadow-md transition">
                        <FaEdit />
                      </button>
                      <button
                        onClick={() => {
          ...
          !reviewsLoading && <p className="text-gray-600 mt-6 text-center text-base">No reviews found</p>
        )}
      </div>
    </div>
  );
